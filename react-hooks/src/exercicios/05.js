...
function Tilt({children}) {
  ...
  React.useEffect(() => {
    ...
    // uma única vez na fase de finalização (unmount) do componente.
    // Este recurso pode ser utilizado para destruir recursos 
    // que foram criados e não são mais necessários (cleanup)
    return () => tiltNode.vanillaTilt.destroy()

  }, []) // quando o vetor de dependências do useEfferct() fica vazio
  // ele é executado apenas uma vez, na 1ª atualização do componente,
